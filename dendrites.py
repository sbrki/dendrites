...
class NeuralNetwork:
	...
	def _Run(self,input):
		...
		if len(input)!= self.Dimensions[0]:
			raise ValueError("The number of providen inputs is not compliant with the network you specified.")
		...
		for i in range(1,len(self.Dimensions)):
			...
			PreviousLayer=np.vstack((self.LayerOutputs[-1],np.ones([1,self.LayerOutputs[-1].shape[1]])))
	...
	def BackPropagationStep(self,input,target,rate=0.2):
		...
		deltas.append(delta_matrice * self.Sigmoid(self.LayerInputs[-1],derivative=True))
		...
		for i in reversed(range(1,len(self.Dimensions)-1)):
			...
			deltas.append(delta_pullback_matrice[:-1:]*self.Sigmoid(self.LayerInputs[i],derivative=True))

		# Compute deltas for the last layer, the "input" layer (neutrons)
		delta_pullback_matrice=np.dot(self.Synapse[0].T,deltas[-1])
		deltas.append(delta_pullback_matrice[:-1:]*self.Sigmoid(self.LayerOutputs[0],derivative=True))
		...
		for i in range(0,len(self.Dimensions)-1):
			LayerOutput=np.vstack((self.LayerOutputs[i], np.ones([1,self.LayerOutputs[i].shape[1]])))

			weightDelta=sum(LayerOutput[None,:,:].transpose(2,0,1)*deltas[i+1][None,:,:].transpose(2,1,0))
	...
	def Train(self,rate=0.02,margin=0.01,max_times=10000,force_convergence=False):
		...
		while (error>margin) and ((count<max_times) or force_convergence):
			count+=1
			old_error=error
			error=self.BackPropagationStep(input=input,target=target,rate=rate)
			#if error>old_error:
